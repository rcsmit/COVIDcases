...
def generate_dummy_data_method3(n_samples, year_range):
    ...
    for year in year_range:
        ...
        for i in range(n_samples):
            ...
            individual_prob = base_mortality_prob + 0.02 * (age[i] > 65) + 0.01 * (medical_history[i] > 1) - 0.01 * (household_income[i] > 50)
            mortality.append(np.random.choice([0, 1], p=[1-individual_prob, individual_prob]))
        
        data.extend(list(zip([year]*n_samples, age, gender, medical_history, migration_background, household_income, mortality)))
    
    return pd.DataFrame(data, columns=['year', 'age', 'gender', 'medical_history', 'migration_background', 'household_income', 'mortality'])
...
def main_claude_method3():
    ...
    # Prepare data for logistic regression
    X_ref = reference_data[['age', 'gender', 'medical_history', 'migration_background', 'household_income']]
    y_ref = reference_data['mortality']

    # Center and scale numerical variables
    scaler = StandardScaler()
    X_ref.loc[:,['age', 'household_income']] = scaler.fit_transform(X_ref[['age', 'household_income']])

    # Convert categorical variables to dummy variables
    X_ref = pd.get_dummies(X_ref, columns=['medical_history', 'migration_background'], drop_first=True)
    ...
    for year in [2020, 2021]:
        year_data = test_data[test_data['year'] == year]
        X_test = year_data[['age', 'gender', 'medical_history', 'migration_background', 'household_income']]
        
        # Apply the same scaling and dummy variable creation as for the reference data
        X_test.loc[:,['age', 'household_income']] = scaler.transform(X_test[['age', 'household_income']])
        X_test = pd.get_dummies(X_test, columns=['medical_history', 'migration_background'], drop_first=True)
    ...
    def analyze_by_phase(data, phase_ranges):
        ...
        for phase, (start, end) in phase_ranges.items():
            ...
            X_phase = phase_data[['age', 'gender', 'medical_history', 'migration_background', 'household_income']]
            # Apply scaling and dummy variable creation
            X_phase.loc[:,['age', 'household_income']] = scaler.transform(X_phase[['age', 'household_income']])
            X_phase = pd.get_dummies(X_phase, columns=['medical_history', 'migration_background'], drop_first=True)
    ...
    phase_results = analyze_by_phase(pd.concat([reference_data, test_data]), pandemic_phases)
...
def main_chatgpt_method2():
    ...
    num_patients = 1000
    data = {
        'age': np.random.normal(70, 10, num_patients),  # Average age around 70 years, std dev of 10
        'gender': np.random.choice([0, 1], size=num_patients),  # 0 for female, 1 for male
        'mortality': np.random.choice([0, 1], size=num_patients, p=[0.9, 0.1])  # 10% mortality rate
    }
...
def main_chatgpt_method3():
    ...
    data = {
        'age': np.random.normal(70, 10, num_patients),  # Average age around 70 years, std dev of 10
        'gender': np.random.choice([0, 1], size=num_patients),  # 0 for female, 1 for male
        'medical_history': np.random.choice([0, 1, 2], size=num_patients, p=[0.5, 0.3, 0.2]),  # 0: No, 1: Acute, 2: Chronic
        'migration_background': np.random.choice([0, 1], size=num_patients, p=[0.8, 0.2]),  # 0: No, 1: Yes
        'household_income': np.random.normal(35000, 10000, num_patients),  # Household income mean of 35k with std dev 10k
        'mortality': np.random.choice([0, 1], size=num_patients, p=[0.9, 0.1])  # 10% mortality rate
    }
    ...
    X = df_reference[['age', 'gender', 'medical_history', 'migration_background', 'household_income']]
    y = df_reference['mortality']
    ...
    new_data = {
        'age': np.random.normal(70, 10, num_patients_2020_2021),
        'gender': np.random.choice([0, 1], size=num_patients_2020_2021),
        'medical_history': np.random.choice([0, 1, 2], size=num_patients_2020_2021, p=[0.5, 0.3, 0.2]),
        'migration_background': np.random.choice([0, 1], size=num_patients_2020_2021, p=[0.8, 0.2]),
...
def main():
    ...
    st.info("Methode 2: **Verwachte sterfte op basis van een logistisch regressiemodel met  leeftijd en geslacht.** In bovenstaande methode werd geen rekening gehouden met verschillen in populatiekenmerken,  daarom is een aanvullende analyse uitgevoerd. Hiervoor is een logistische regressie analyse  uitgevoerd over de referentiejaren (2015-2019) met overlijden als uitkomstmaat en als  determinanten leeftijd (gecentreerd rondom het gemiddelde) en geslacht. Voor elke patiënt in 2020  en 2021 zijn nieuwe coëfficiënten berekend aan de hand van de coëfficiënten (leeftijd en geslacht)  uit de referentiejaren, hiervoor werd een random waarde getrokken uit de normale verdeling van de  coëfficiënten over de referentiejaren. Vervolgens is voor elke patiënt de regressieformule met de  nieuwe coëfficiënten ingevuld, waarmee op patiëntniveau de kans op overlijden werd berekend. Om  de kans op overlijden voor de gehele populatie te bepalen is het gemiddelde genomen over alle  patiënten. Om te bepalen of er sprake was van oversterfte werd gekeken of de werkelijke sterfte (in  percentage) onder (ondersterfte), binnen (geen over- of ondersterfte) of boven (oversterfte) het 95%  betrouwbaarheidsinterval (in percentage) lag van de verwachte kans op overlijden. Dit is bepaald per  jaar, maar ook voor de verschillende fases van de pandemie (zie 2.3). Hierdoor werden dezelfde  periodes met elkaar vergeleken en werd gecorrigeerd voor seizoenseffecten.De werkelijke sterfte  werd vergeleken met de bovenkant van het 95%-betrouwbaarheidsinterval van de verwachte sterfte  en met de gemiddelde verwachte sterfte, waardoor de uitkomst een range van over- of ondersterfte  werd.  ")
    main_claude_method2()
    main_chatgpt_method2()  
    st.info("""Methode 3: In deze laatste methode is het bovenstaande regressiemodel verder uitgebreid met aanvullende
