...
def main_chatgpt():
    ...
    def fit_baseline(df, season_year_target: int, filter_all:float, filter_summer:float, harmonics: int = 1):
        ...
        st.plotly_chart(fig_4, key=f"fig_train_{random.randint(1,10000)}")
    ...
    def make_plot_all_seasons(df, seasons, include_baselines=False, harmonics=1):
        ...
        for sy in seasons:
            ...
            if include_baselines:
                try:
                    #with st.expander("Baselines per jaar"):
    ...
    with tab2:
        ...
        # try:
        if 1==1:
            result = fit_baseline(df, season_choice,filter_all, filter_summer, harmonics=2 if use_harm2 else 1)
            target_df = result["target_df"]
            fig_7 = make_plot_single(target_df, season_choice)
...
""")
    ...
    st.markdown("**ad. 3) Seizoenseffect 2 harmonische**")
    st.latex(r"y = a + b\,t + c\,\sin\!\left(\tfrac{2\pi t}{52}\right) + d\,\cos\!\left(\tfrac{2\pi t}{52}\right) + e\,\sin\!\left(\tfrac{4\pi t}{52}\right) + f\,\cos\!\left(\tfrac{4\pi t}{52}\right)")
...
Daardoor blijven lagere weken uit 2022–2025 relatief zwaarder wegen.""")
    st.info("""Methods for Current Statistical Analysis of Excess Pneumonia-Influenza Deaths
Author(s): Robert E. Serfling
Source: Public Health Reports (1896-1970), Vol. 78, No. 6 (Jun., 1963), pp. 494-506
